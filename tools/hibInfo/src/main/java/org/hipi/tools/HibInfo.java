...
public class HibInfo {
	...
	private static void usage() {
		...
		formatter.printHelp(
				"hibInfo.jar <input > [--show-exif] [--show-meta] [#index [--extract file.png] [--meta key]]",
				options);
		System.exit(0);
	}
	...
	private static void displayImageHeader(HipiImageHeader header, boolean showMeta, boolean showExif) {
		...
		if (showMeta) {
			...
			System.out.println("FileName " + metaData.get("filename"));
		}
...
//				hib.currentImage()
			...
			if (!hib.next()) {
				System.err.println(String.format("Failed to locate image with index [" + imageIndex
						+ "]. Check that HIB contains sufficient number of images."));
				System.exit(0);
			}
			...
			if (extractImagePath != null) {
				...
				if (imageExtension == null) {
					System.err.println(String.format(
							"Failed to determine image type based on extension [%s]. Please provide a valid path with complete extension.",
							extractImagePath));
					System.exit(0);
				}
				...
				if (writers == null) {
					System.err.println(
							String.format("Failed to locate encoder for image extension [%s]", imageExtension));
					System.exit(0);
				}
