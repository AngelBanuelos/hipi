...
/**
 * A HipiImageBundle (HIB) is the primary representation for a collection of
 * images on the Hadoop Distributed File System (HDFS) used by HIPI.
 *
 * HIBs are designed to take advantage of the fact that Hadoop MapReduce is
 * optimized to support efficient processing of large flat files.
 *
 * This class provides basic methods for writing, reading, and concatenating
 * HIBs.
 * 
 * A single HIB is actually comprised of two files stored on the file system: an
 * index file and a data file. The index file contains a list of byte offsets to
 * the end of each image record (header metadata + image pixel data) in the data
 * file. The data file is composed of a contiguous sequence of image records.
 *
...
public class HipiImageBundle {
	...
	/**
	 * This FileReader enables reading individual images from a
	 * {@link org.hipi.imagebundle.HipiImageBundle} and delivers them in the
	 * specified image type. This class is used by the
	 * {@link org.hipi.imagebundle.mapreduce.HibInputFormat} and
	 * {@link org.hipi.imagebundle.mapreduce.HibRecordReader} classes.
	 */
	public static class HibReader {
		...
		/**
		 * Creates a HibReader to read records (image headers / image bodies)
		 * from a contiguous segment (file split) of a HIB data file. The
		 * segment is specified by a start and end byte offset.
		 * 
		 * @param fs
		 *            The {@link FileSystem} where the HIB data file resides
		 * @param path
		 *            The {@link Path} to the HIB data file
		 * @param start
		 *            The byte offset to beginning of segment
		 * @param end
		 *            The byte offset to end of segment
		 *
		 * @throws IOException
		 */
		public HibReader(HipiImageFactory imageFactory, Class<? extends Culler> cullerClass, FileSystem fs, Path path,
			...
			if (cullerClass != null) {
				try {
					...
					System.err.println(
							"Fatal error while attempting to instantiate image culler: " + cullerClass.getName());
		...
		public HibReader(HipiImageFactory imageFactory, Class<? extends Culler> cullerClass, FileSystem fs, Path path)
				throws IOException {
			this(imageFactory, cullerClass, fs, path, 0, 0); // endOffset = 0
																// indicates
																// read until
																// EOF
		}
		 ...
		 * 
		 * @return Measure of progress from 0.0 (no progress) to 1.0 (finished).
		 */
		public float getProgress() {
			float progress = (endOffset - startOffset + 1) > 0
					? (float) (currentOffset - startOffset) / (float) (endOffset - startOffset + 1) : 0.f;
		...
		/**
		 * Reads the next image header and image body into memory. To obtain the
		 * corresponding {@link org.hipi.image.HipiImageHeader} and
		 * {@link org.hipi.image.RasterImage} objects, call
		 * {@link #getCurrentKey()} and {@link #getCurrentValue()} respectively.
		 * 
		 * @return true if the next image record (header + pixel data) was
		 *         successfully read and decoded. False if there are no more
		 *         images or if an error occurs.
		 */
		public boolean nextKeyValue() {
			...
			try {
				...
				if (bytesRead < sig.length) {
					...
					// throw new IOException(String.format("Failed to read
					// %d-byte HIB image signature that delineates image record
					// boundaries.", sig.length));
				}
...
//					throw new IOException("Found image header length <= 0 in HIB at offset: " + currentOffset);
				...
				int imageFormatInt = ((sig[8] & 0xff) << 24) | ((sig[9] & 0xff) << 16) | ((sig[10] & 0xff) << 8)
						| (sig[11] & 0xff);
				try {
					...
					return true;
					// TODO remove next commented line
					// throw new IOException("Found invalid image storage format
...
//					throw new IOException("Found UNDEFINED image storage format in HIB at offset: " + currentOffset);
				 ...
				 * System.out.println("imageHeaderLength: " +
				 * imageHeaderLength); System.out.println("imageLength: " +
				 * imageLength); System.out.println("imageFormatInt: " +
				 * imageFormatInt); System.out.println("imageFormat: " +
				 * imageFormat.toInteger());
				 */
				...
				System.err.println(
						String.format("EOF exception [%s] while decoding HIB image record ending at byte offset [%d]",
				...
				System.err.println(
						String.format("IO exception [%s] while decoding HIB image record ending at byte offset [%d]",
				...
				System.err.println(String.format(
						"Runtime exception [%s] while decoding HIB image record ending at byte offset [%d]",
						e.getMessage(), currentOffset));
				e.printStackTrace();
				imageFormat = HipiImageFormat.UNDEFINED;
				imageBytes = null;
				...
				System.err.println(String.format(
						"Unexpected exception [%s] while decoding HIB image record ending at byte offset [%d]",
						e.getMessage(), currentOffset));
				e.printStackTrace();
				imageFormat = HipiImageFormat.UNDEFINED;
				imageBytes = null;
		...
		/**
		 * @return Header for the current image, as retrieved by
		 *         {@link #nextKeyValue()}
		 */
		...
		/**
		 * @return Current decoded image, as retrieved by
		 *         {@link #nextKeyValue()}
		 */
	 ...
	 * 
	 * @param overwrite
	 *            if either part of the HIB file (index and/or data) exists this
	 *            parameter determines whether or not to delete the file first
	 *            or throw an exception
	 *
	 * @throws IOException
	 *             in the event of any I/O errors while creating and opening the
	 *             index and data files for subsequent writing
	 */
	public final void openForWrite(boolean overwrite) throws IOException {

		if (fileMode != FILE_MODE_UNDEFINED) {
			throw new IOException(
					"HIB [" + indexFilePath.getName() + "] is already open. Must close before calling this method.");
		}
		...
		if (fs.exists(indexFilePath) && !overwrite) {
			throw new IOException("HIB [" + indexFilePath.getName()
					+ "] already exists. Cannot open HIB for writing unless overwrite is specified.");
		}

		if (fs.exists(dataFilePath) && !overwrite) {
			throw new IOException("HIB [" + dataFilePath.getName()
					+ "] already exists. Cannot open HIB for writing unless overwrite is specified.");
		}
		...
		if (blockSize <= 0) {
			...
			// System.out.println("HIPI: Using default blockSize of [" +
			// blockSize + "].");
		}

		if (replication <= 0) {
			replication = fs.getDefaultReplication(dataFilePath);
			// System.out.println("HIPI: Using default replication factor of ["
			// + replication + "].");
		}

		try {
			if (!overwrite && fs.exists(indexFilePath) && fs.exists(dataFilePath)) {
				// Appending => open for write and seek to end of index and data
				// files
				throw new IOException("Not implemented.");
			} else {
				// Begin from scratch either because HIB doesn't yet exist or
				// because an explicit overwrite was requested
				indexOutputStream = new DataOutputStream(fs.create(indexFilePath));
				dataOutputStream = new DataOutputStream(fs.create(dataFilePath, true,
						fs.getConf().getInt("io.file.buffer.size", 4096), replication, blockSize));
				currentOffset = 0;
				writeBundleHeader();
			}
		} catch (IOException ex) {
			System.err.println("I/O exception while attempting to open HIB [" + indexFilePath.getName()
					+ "] for writing with overwrite [" + overwrite + "].");
	...
	/**
	 * HIB index file header structure: BOF 4 bytes (int): magic signature
	 * (0x81911618) "HIPIIbIH" 2 bytes (short int): length of data file name var
	 * bytes: data file path name 16 bytes: reserved for future use 4 bytes:
	 * number of bytes to skip to reach start of offset list [8 byte]*: offsets
	 * EOF
	 */
	...
	/**
	 * Add image to the HIB. This involves appending the image to the data file,
	 * and adding the corresponding byte offset to the index file.
	 *
	 * @param imageHeader
	 *            initialized image header
	 * @param imageStream
	 *            input stream containing the image data. This data is not
	 *            decoded or verified to be consistent with the provided image
	 *            header. It is simply appended to the HIB data file.
	 *
	 * @throws IOException
	 *             in the event of any I/O errors or if the HIB is not currently
	 *             in a state that supports adding new images
	 */
	public void addImage(HipiImageHeader imageHeader, InputStream imageStream) throws IOException {
		...
		/*
		 * // debug System.out.println("addImage()");
		 ...
		 * imageHeaderBytes[0], imageHeaderBytes[1], imageHeaderBytes[2],
		 * imageHeaderBytes[3]);
	...
	public void addImage(InputStream inputStream, HipiImageFormat imageFormat, HashMap<String, String> metaData)
		...
		case UNDEFINED:
			defult: throw new IllegalArgumentException("Unrecognized or unsupported image format.");
		}
	...
	public void openForRead(int seekToImageIndex) throws IOException, IllegalArgumentException {
		...
		if (!fs.exists(indexFilePath)) {
			throw new IOException(
					"HIB index file not found while attempting open for read [" + indexFilePath.getName() + "].");
		}

		if (!fs.exists(dataFilePath)) {
			throw new IOException(
					"HIB data file not found while attempting open for read [" + dataFilePath.getName() + "].");
		}
		...
		try {
			...
			System.err.println(
					"I/O exception while attempting to open HIB [" + indexFilePath.getName() + "] for reading.");
			System.err.println(ex.getMessage());
			indexOutputStream = null;
			dataOutputStream = null;
			indexInputStream = null;
		...
		if (seekToImageIndex != 0) {
			...
			if (offsets.size() != seekToImageIndex) {
				throw new IOException("Failed to seek to image index [" + seekToImageIndex
						+ "]. Check that it is not past end of file.");
			}
		}
	...
	/**
	 * Attemps to read some number of image record offsets from the HIB index
	 * file.
	 *
	 * @param maximumNumber
	 *            the maximum number of offsets that will be read from the HIB
	 *            index file. The actual number read may be less than this
	 *            number.
	...
	public boolean next() throws IOException {
		if (imageFactory == null) {
			throw new RuntimeException(
					"Must provide a valid image factory to the HipiImageBundle constructor in order to call this method.");
		}
		if (fileMode != FILE_MODE_READ) {
			throw new IOException("HIB [" + indexFilePath.getName()
					+ "] is not opened for reading. Must successfully open HIB for reading before calling this method.");
		}
	...
	/**
	 * Appends another HIB to the current HIB. This involves concatenating the
	 * underlying data files and index files.
	 * 
	 * @param bundle
	 *            target HIB to be appended to the current HIB
	 */
